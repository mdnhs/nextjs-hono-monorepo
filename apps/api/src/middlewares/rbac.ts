...
import { db } from '../db'
import { stores, products, orders, reviews } from '../db/schema'
...
export const requirePermission = (...permissions: Permission[]) =>
  async (c: Context, next: Next) => {
    ...
    if (!allowed) throw new HTTPException(403, { message: 'Insufficient permissions' })
    await next()
  }

// Verifies the authenticated seller owns the store identified by :storeId or :id param.
// ADMIN bypasses this check.
export const requireStoreOwnership = async (c: Context, next: Next) => {
  const user = c.get('user')
  ...
  if (store.ownerId !== user.userId) {
    throw new HTTPException(403, { message: 'Access denied: not your store' })
  }
